<?php
	...
	while($row = $query->fetch_assoc()){
		...
		$bosql = "SELECT *, SUM(amount) AS cashamount FROM bonus WHERE employee_id='$empid' AND date_bonus BETWEEN '$from' AND '$to'";
                      
        $boquery = $conn->query($bosql);
        $borow = $boquery->fetch_assoc();
        $bonus = $borow['cashamount'];

		$gross = $row['rate'] * $row['total_hr'];
		$total_deduction = $deduction + $cashadvance;
  		$net = $gross + $bonus - $total_deduction;

		$contents .= '
			<h2 align="center">Tracker Pay</h2>
			...
			<table cellspacing="0" cellpadding="3">  
				...
				<tr> 
    	    		<td></td> 
    	    		<td></td>
				 	<td width="25%" align="right">Bonus: </td>
				 	<td width="25%" align="right">'.number_format($bonus, 2).'</td> 
				</tr>
    	    	<tr> 
    	    		<td></td> 
    	    		<td></td>
